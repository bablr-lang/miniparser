...
export class Printer {
  constructor(language, options = defaultOptions) {
    this.grammars = [];
  ...
  eatProduction(node, type = node.type) {
    const { grammar, options } = this;
    return getPrototypeOf(grammar)[type].call(
      grammar,
      this,
      type === node.type ? getNode(node, options) : node,
    );
  }
...
export const print = (grammar, node, type = node.type, options = defaultOptions) => {
  const printer = new Printer(grammar, options);
